 ...
 insert overwrite table dim.dim_user_zip partition (dt = '20250718')
 select data.id,
 ...
 insert overwrite table dim.dim_activity_full partition(`dt`='20220213')
 select
     rule.id,
     info.id,
     activity_name,
 ...
 select * from dim.dim_activity_full;
 ...
 insert overwrite table dim.dim_promotion_refer_full partition(`dt`='20211214')
 select
     `id`,
     `refer_name`,
     `create_time`,
 ...
 select * from dim.dim_promotion_refer_full;

 --todo dim.dim_province_full;
 ...
 insert overwrite table dim.dim_province_full partition(`dt`='20211214')
 select
     province.id,
     province.name,
     province.area_code,
 ...
 select * from dim.dim_province_full;

 --todo dim.dim_sku_full;
 ...
 insert overwrite table dim.dim_sku_full partition(`ds`='20211214')
 select
     sku.id,
     sku.price,
     sku.sku_name,
 ...
 DROP TABLE IF EXISTS dim.tmp_dim_date_info;
 CREATE EXTERNAL TABLE dim.tmp_dim_date_info (
 ...
 LOCATION '/bigdata_warehouse/gmall/dim/tmp_dim_date_info/';



 load data local inpath '/zuoye/date_info.txt' overwrite into table dim.tmp_dim_date_info;
 insert overwrite table dim.dim_date select * from dim.tmp_dim_date_info;

 select * from dim.dim_date;

 -- todo dim.dim_coupon_full;
 ...
 insert overwrite table dim.dim_coupon_full partition(`dt`='20250718')
 select
     id,
     coupon_name,
     coupon_type,
 ...
 select * from dim.dim_coupon_full;


 -- todo dim.dim_promotion_pos_full;
 ...
 insert overwrite table dim.dim_promotion_pos_full partition(`dt`='20211214')
 select
     `id`,
     `pos_location`,
     `pos_type`,
 ...
 select * from dim.dim_promotion_pos_full;
