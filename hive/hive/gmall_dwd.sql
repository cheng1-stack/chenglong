...
insert overwrite table dwd.dwd_trade_cart_full partition(dt='20250718')
select
    id,
    user_id,
    sku_id,
...
select * from  dwd.dwd_trade_cart_full;
...
select * from dwd.dwd_trade_cart_add_inc;


-- todo dwd.dwd_tool_coupon_used_inc;
...
insert overwrite table dwd.dwd_tool_coupon_used_inc partition(dt='20250718')
select
    data.id,
    data.coupon_id,
    data.user_id,
...
insert into table dwd.dwd_tool_coupon_used_inc partition(dt='20250718')
select
    data.id,
    data.coupon_id,
    data.user_id,
...
select * from dwd.dwd_tool_coupon_used_inc;

-- todo dwd.dwd_interaction_favor_add_inc;
...
insert overwrite table dwd.dwd_interaction_favor_add_inc partition(dt='20250718')
select
    data.id,
    data.user_id,
    data.sku_id,
...
from ods_favor_info data
where ds='20250718';
select * from dwd.dwd_interaction_favor_add_inc;


-- todo dwd.dwd_user_register_inc;
...
insert overwrite table dwd.dwd_user_register_inc partition(dt='20220608')
select
    ui.user_id,
    date_format(ui.create_time,'yyyy-MM-dd') date_id,
    ui.create_time,
...
select * from dwd.dwd_user_register_inc;


-- todo dwd.dwd_user_login_inc;
...
insert overwrite table dwd.dwd_user_login_inc partition (dt = '20250718')
select
    user_id,
...
select * from dwd.dwd_user_login_inc;

-- todo dwd.dwd_traffic_page_view_inc;
...
insert overwrite table dwd.dwd_traffic_page_view_inc partition (dt='20250718')
select
    common.province_id,
    common.brand,
    common.channel,
...
select * from dwd.dwd_traffic_page_view_inc;

-- todo dwd.dwd_trade_trade_flow_acc;
...
select
    ...
    nvl(date_format(finish_time,'yyyy-MM-dd'),'9999-12-31') as ds  -- 添加别名
from
(
    select
        data.id,
...
select
    ...
    nvl(nvl(oi.finish_time,log.finish_time),'9999-12-31') as ds  -- 添加别名
from
(
    -- 历史数据部分（保持不变）
    select
    ...
    -- 新增数据部分（添加类型转换）
    select
        cast(data.id as string) as order_id,  -- 转换为STRING类型
        data.user_id,
        data.province_id,
        date_format(data.create_time,'yyyy-MM-dd') order_date_id,
        cast(data.create_time as string) as order_time,  -- 转换为STRING类型
...
(
    select
        cast(data.order_id as string) as order_id,  -- 转换为STRING类型
        date_format(data.callback_time,'yyyy-MM-dd') payment_date_id,
        cast(data.callback_time as string) as payment_time,  -- 转换为STRING类型
        data.total_amount payment_amount
    from ods_payment_info data
    where ds='20250718'
    and data.payment_status='1602'
...
(
    select
        ...
        cast(data.create_time as string) as finish_time -- 转换为STRING类型
    from ods_order_status_log data
    where ds='20250718'
    and data.order_status='1004'
)log
...
-- 查看结果
select * from dwd.dwd_trade_trade_flow_acc;



-- todo dwd.dwd_trade_pay_detail_suc_inc;
...
insert overwrite table dwd.dwd_trade_pay_detail_suc_inc partition (dt='20250718')
select
    od.id,
    od.order_id,
    user_id,
...
(
    select
        ...
        data.split_coupon_amount
    from ods_order_detail data
    where (ds = '20250718' or ds = date_add('20250718',-1))
) od
join
(
    select
...
select * from dwd.dwd_trade_pay_detail_suc_inc;

-- todo dwd.dwd_trade_order_detail_inc;
...
select * from dwd.dwd_trade_order_detail_inc;